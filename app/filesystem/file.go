...
func NewFile(file string) (*File, error) {
	...
	fmt.Println("read------------------")
	disk, _ := config.ReadConfig("filesystems.default")
	fmt.Println(disk)
...
func NewFileFromRequest(fileHeader *multipart.FileHeader) (*File, error) {
	...
	if err != nil {
		if src, err = os.Open(fileHeader.Filename); err != nil {
			return nil, err
		}
	}
	defer src.Close()

	appName, _ := config.ReadConfig("app.name")
	tempFileName := fmt.Sprintf("%s_*%s", appName, path.Ext(fileHeader.Filename))
