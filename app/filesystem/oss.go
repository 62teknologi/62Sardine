...
func NewOss(ctx context.Context, disk string, contenType string, isPublic bool) (*Oss, error) {
	accessKeyId, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.key", disk))
	accessKeySecret, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.secret", disk))
	endpoint, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.endpoint", disk))
	bucket, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.bucket", disk))
...
func (r *Oss) Put(file string, content string) error {
	...
	pub := oss.ACLPrivate
	if r.isPublic {
		pub = oss.ACLPublicRead
	}

	ops := []oss.Option{oss.ContentType(r.contenType), oss.ObjectACL(pub)}

	return r.bucketInstance.PutObjectFromFile(file, tempFile.Name(), ops...)
}
...
func (r *Oss) PutFileAs(filePath string, source filesystem.File, name string) (string, error) {
	...
	if err := r.bucketInstance.PutObjectFromFile(fullPath, source.File(), ops...); err != nil {
		return "", err
	}
...
func (r *Oss) WithContext(ctx context.Context) filesystem.Driver {
	driver, err := NewOss(ctx, r.disk, "", false)
	if err != nil {
