...
func NewS3(ctx context.Context, disk string, contenType string, isPublic bool) (*S3, error) {
	...
	region, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.region", disk))
	...
	cfg, _ := config2.LoadDefaultConfig(ctx,
		config2.WithRegion(region),
		config2.WithCredentialsProvider(creds),
		config2.WithEndpointResolverWithOptions(customResolver))
...
func (r *S3) WithContext(ctx context.Context) filesystem.Driver {
	driver, err := NewS3(ctx, r.disk, "", false)
	if err != nil {
