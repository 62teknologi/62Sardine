...
func NewMinio(ctx context.Context, disk string) (*Minio, error) {
	key, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.key", disk))
	secret, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.secret", disk))
	...
	diskUrl, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.url", disk))
	ssl, _ := config.ReadConfig(fmt.Sprintf("filesystems.disks.%s.ssl", disk))
